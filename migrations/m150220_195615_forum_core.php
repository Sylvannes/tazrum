...
class m150220_195615_forum_core extends Migration
{
    public function up()
    {
        ...
        $this->createTable('{{%user}}', [
            ...
            'shoutrpg_wins' => 'INT(11) NOT NULL DEFAULT \'0\'',
            'shoutrpg_losses' => 'INT(11) NOT NULL DEFAULT \'0\'',
        ...
        $this->addForeignKey('fk_topic_post', '{{%post}}', 'topic_id', '{{%topic}}', 'id', 'CASCADE', 'CASCADE' );
        $this->addForeignKey('fk_user_post', '{{%post}}', 'user_id', '{{%user}}', 'id', 'CASCADE', 'CASCADE' );
        $this->addForeignKey('fk_last_edited_user_post', '{{%post}}', 'last_edited_by_user_id', '{{%user}}', 'id', 'CASCADE', 'CASCADE' );
        ...
        $this->addForeignKey('fk_post_post_read', '{{%post_read}}', 'post_id', '{{%post}}', 'id', 'CASCADE', 'CASCADE' );
        $this->addForeignKey('fk_topic_post_read', '{{%post_read}}', 'topic_id', '{{%topic}}', 'id', 'CASCADE', 'CASCADE' );
        $this->addForeignKey('fk_user_post_read', '{{%post_read}}', 'user_id', '{{%user}}', 'id', 'CASCADE', 'CASCADE' );
        $this->createIndex('unique', '{{%post_read}}', ['user_id', 'topic_id', 'post_id'], true);

        // FK and indices for shout
        $this->addForeignKey('fk_user_shout', '{{%shout}}', 'user_id', '{{%user}}', 'id', 'CASCADE', 'CASCADE' );
        $this->addForeignKey('fk_to_user_shout', '{{%shout}}', 'to_user_id', '{{%user}}', 'id', 'CASCADE', 'CASCADE' );
        $this->createIndex('created_on', '{{%shout}}', 'created_on');

        // FK and indices for subforum
        $this->addForeignKey('fk_category_subforum', '{{%subforum}}', 'category_id', '{{%category}}', 'id', 'CASCADE', 'CASCADE' );
        $this->addForeignKey('fk_topic_subforum', '{{%subforum}}', 'last_topic_id', '{{%topic}}', 'id', 'CASCADE', 'CASCADE' );

        // FK and indices for topic
        $this->addForeignKey('fk_post_topic', '{{%topic}}', 'last_post_id', '{{%post}}', 'id', 'CASCADE', 'CASCADE' );
        $this->addForeignKey('fk_subforum_topic', '{{%topic}}', 'subforum_id', '{{%subforum}}', 'id', 'CASCADE', 'CASCADE' );
        $this->addForeignKey('fk_user_topic', '{{%topic}}', 'user_id', '{{%user}}', 'id', 'CASCADE', 'CASCADE' );
        $this->addForeignKey('fk_last_post_user_topic', '{{%topic}}', 'last_post_user_id', '{{%user}}', 'id', 'CASCADE', 'CASCADE' );
        $this->createIndex('private', '{{%topic}}', 'private');
        $this->createIndex('last_post_on', '{{%topic}}', 'last_post_on');

        // FK and indices for user
        $this->addForeignKey('fk_banned_user_user', '{{%user}}', 'banned_by_user_id', '{{%user}}', 'id', 'CASCADE', 'CASCADE' );
        $this->addForeignKey('fk_deleted_user_user', '{{%user}}', 'deleted_by_user_id', '{{%user}}', 'id', 'CASCADE', 'CASCADE' );

        // FK and indices for user_salt
        $this->addForeignKey('fk_user_user_salt', '{{%user_salt}}', 'user_id', '{{%user}}', 'id', 'CASCADE', 'CASCADE' );
